...
open class HTTPFileProvider: NSObject, FileProviderBasicRemote, FileProviderOperations, FileProviderReadWrite, FileProviderReadWriteProgressive {
    ...
    internal fileprivate(set) var sessionDelegate: FilesProviderSessionDelegate?
    public var session: URLSession {
        get {
            if _session == nil {
                self.sessionDelegate = FilesProviderSessionDelegate(fileProvider: self)
        ...
        set {
            assert(newValue.delegate is FilesProviderSessionDelegate, "session instances should have a SessionDelegate instance as delegate.")
            _session = newValue
            if _session.sessionDescription?.isEmpty ?? true {
                _session.sessionDescription = UUID().uuidString
            }
            self.sessionDelegate = newValue.delegate as? FilesProviderSessionDelegate
            initEmptySessionHandler(_session.sessionDescription!)
        }
    }
